...
def email_is_configured():
    """True nếu đã có email + app password hợp lệ (không phải placeholder)."""
    c = load_email_config()
    email = c["email"]
    pw = c["app_password"]
...
def send_otp_email(to_email, otp_code):
    """Gửi mã OTP qua Gmail. Trả về (ok, message)."""
    c = load_email_config()
    smtp_email = c["email"]
...
if __name__ == "__main__":
    ...
    if email_is_configured():
        c = load_email_config()
        print(f"✅ Email gửi OTP đã cấu hình: {c['email']}")
    else:
        print("⚠️  CHƯA cấu hình email gửi OTP!")
        print(f"   Tạo/sửa file: {EMAIL_CONFIG_FILE}")
        print('   Nội dung: {"email":"ban@gmail.com","app_password":"app password 16 ky tu"}')
        print("   (Xem hướng dẫn lấy App Password trong README_EMAIL.txt)")
    print()
