...
def display_name_from_email(email):
    """Tạo tên hiển thị từ email: lấy 2 chữ cái ĐẦU của phần trước @.
    Ví dụ: kl5823633@gmail.com -> 'kl', uliueke@gmail.com -> 'ul'.
    """
    local = (email or "").split("@")[0]
    letters = re.sub(r"[^a-zA-Z]", "", local)  # bỏ số và ký tự đặc biệt
    name = letters[:2] or local[:2] or "user"
    return name.lower()
...
# ===== ĐĂNG NHẬP GOOGLE =====
# Cần một "OAuth Client ID" (loại Web application) lấy ở Google Cloud Console.
# Client ID KHÔNG phải bí mật (nó lộ ngay trong trình duyệt) nên có thể để trong file.
# Đọc từ biến môi trường GOOGLE_CLIENT_ID hoặc file google_config.json: {"client_id": "..."}.
GOOGLE_CONFIG_FILE = os.path.join(ROOT, "google_config.json")
...
def get_google_client_id():
    ...
    if os.path.exists(GOOGLE_CONFIG_FILE):
        try:
            ...
            print(f"[GOOGLE] Lỗi đọc {GOOGLE_CONFIG_FILE}: {e}")
    return ""


def verify_google_token(credential):
    """Xác minh ID token của Google qua endpoint tokeninfo. Trả về dict thông tin hoặc None."""
    if not credential:
        return None
    ...
    try:
        ...
        print(f"[GOOGLE] Xác minh token thất bại: {e}")
    return None
...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == "/auth/google/config":
            return self.send_json(200, {"client_id": get_google_client_id()})
    ...
    def _handle_register(self):
        ...
        # Tên hiển thị tự sinh từ email (2 chữ cái đầu, vd uliueke@... -> 'ul')
        username = display_name_from_email(email)
    ...
    def _handle_google(self):
        ...
        if not client_id:
            return self.send_json(503, {"error": "Server chưa cấu hình đăng nhập Google (thiếu Client ID)."})

        data = self._read_json()
        credential = data.get("credential") or ""
        info = verify_google_token(credential)
        if not info:
            return self.send_json(401, {"error": "Token Google không hợp lệ"})

        # Token phải được cấp cho ĐÚNG ứng dụng của ta
        if info.get("aud") != client_id:
            return self.send_json(401, {"error": "Token Google không dành cho ứng dụng này"})

        email = (info.get("email") or "").strip().lower()
        email_verified = str(info.get("email_verified", "")).lower() == "true"
        if not email or not email_verified:
            return self.send_json(400, {"error": "Email Google chưa được xác minh"})
        ...
        if existing:
            ...
            # Tài khoản mới qua Google: tên theo cùng quy tắc (2 chữ cái đầu của email)
            username = display_name_from_email(email)
            db["users"][email] = {
                "username": username,
                "password": "",          # đăng nhập Google, không có mật khẩu
                "email": email,
                "created": time.time(),
                "google": True,
            }
        ...
        token = create_session(username)
        self.send_json(200, {"message": "Đăng nhập Google thành công!", "token": token, "username": username})
